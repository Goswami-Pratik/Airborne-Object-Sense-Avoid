...
class Flight:
    ...
    def generate_video(self):
        ...
        if not os.path.isfile(flight_folder + 'flight.mp4'):
            ...
            os.system("ffmpeg -y -r 10 -f concat -safe 0 -i output.txt -c:v libx264 -pix_fmt yuv420p flight.mp4")
            # os.system(" ".join([
            #     'ffmpeg', '-y', '-r', str(10*speed_x), '-f  concat safe 0', '-i', 'output.txt', '-c:v', 'libx264',
            #     '-pix_fmt', 'yuv420p', 'flight.mp4'
            # ]))
